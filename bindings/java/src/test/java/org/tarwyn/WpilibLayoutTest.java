...
final class WpilibLayoutTest {
    ...
    @Test
    void a_pose3d_packed_from_its_getters_is_byte_identical_to_wpilibs() {
        ...
        ByteBuffer ours = little(7 * 8);
        ours.putDouble(pose.getX()).putDouble(pose.getY()).putDouble(pose.getZ())
            .putDouble(q.getW()).putDouble(q.getX()).putDouble(q.getY()).putDouble(q.getZ());

        assertEquals(wpilib.rewind(), ours.rewind(), "the two layouts must be byte-identical");
    }
}
