...
///
/// The publisher starts once the subscriber says it is waiting. TARWYN says so
/// before its subscribe socket has finished connecting, so that probe is also
/// waited on for the connection itself.
fn run_case(
    env: &Env,
    settings: &Settings,
    cores: &Cores,
    ...
    if plan.probe == Probe::JavaClient {
        wait_for_marker(&out, "Client socket connected: SUBSCRIBE", ready);
    }
            ...
            for &payload in &settings.payloads {
                for case in catalog::CASES {
                    ...
                    for implementation in case.implementations {
                        if *implementation == "tarwyn" && env.java_classpath.is_none() {
                            continue;
                        }
                        eprintln!(
