...
/// paced the send. The server gets every other physical core: all three
/// servers are multi-threaded, and one core starves them: the TARWYN JVM
/// dropped one to two per cent of messages pinned that way and none unpinned.
/// Core 0 and its siblings are skipped throughout.
///
...
impl Cores {
    ...
    /// pinned at all. A probe whose transport runs on threads of its own gets
    /// the machine instead; see [`super::plan::Probe::pinnable`].
